...
#     E.g.: docker.io/hexpm/elixir:1.19.5-erlang-26.2.5.21-debian-trixie-20260824-slim
...
COPY --from=builder --chown=nobody:root /app/_build/${MIX_ENV}/rel/hermes ./
...
# Run pending migrations on every start, so a customer rollout is just `docker compose up`.
