defmodule HermesWeb.UserAuthTest do
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, HermesWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      HermesWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "require_authenticated_user/2" do
    ...
    test "redirects if user is not authenticated", %{conn: conn} do
      ...
      assert Phoenix.Flash.get(conn.assigns.flash, :error) ==
               "Bitte melde dich an, um diese Seite zu öffnen."
    end
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        HermesWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
